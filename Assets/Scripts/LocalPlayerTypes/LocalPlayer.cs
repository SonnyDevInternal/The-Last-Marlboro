...
public class LocalPlayer : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            float perc = this.owningPlayer.GetStamina() / this.owningPlayer.GetMaxStamina();

            this.staminaBar.SetPercentage(perc);
        }

        if(this.hasHealthSliderUI)
        {
            float perc = (float)this.owningPlayer.GetHealth() / (float)this.owningPlayer.GetMaxHealth();
    ...
    protected void SetShowUi(bool value)
    {
        ...
        {
            this.hasHealthSliderUI = value;
            this.healthBar.enabled = value;
        }
        else
            this.hasHealthSliderUI = false;
    }
    ...
    virtual protected void OnUpdateClientCamera()
    {
        ...
        if (translation != 0.0f)
        {
            ...
            this.owningPlayer.OnClientRotate(addEuler);
        }
    }
